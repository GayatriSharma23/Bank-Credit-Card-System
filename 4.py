...
print("\n✅ All batches processed. Now applying fuzzy matching to align with input data...")

# Function to match transactions based on similarity
def find_best_match(narration, transaction_list):
    match = difflib.get_close_matches(narration, transaction_list, n=1, cutoff=0.5)
    return match[0] if match else None

# Apply fuzzy matching to align LLM responses with original data
df['Matched_Transaction'] = df[TRANSACTION_COLUMN].apply(lambda x: find_best_match(x, categories_df_all['Transaction'].tolist()))
df = df.merge(categories_df_all, left_on='Matched_Transaction', right_on='Transaction', how='left').drop(columns=['Matched_Transaction', 'Transaction'])

# Save the categorized transactions
output_file = "categorized_transaction_mixtral_fuzzy.csv"
